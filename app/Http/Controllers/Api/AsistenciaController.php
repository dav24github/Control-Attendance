...
class AsistenciaController extends Controller
{
    public function asistenciaStoreManual(Request $request){
        ...
        if($practicante){               
            ...
            $asistencia->horario_details_id =$request->horario_details_id;    
            $asistencia->h_entrada = $hr_e->format('H:i:s');
            $asistencia->h_salida = $hr_s->format('H:i:s');
            $asistencia->retraso = "";
                
            $asistencia->asist_fecha = $request->fecha;
            $asistencia->estado = 1;
    
            $asistencia->asist_mes =date("F", strtotime($request->fecha));
            $asistencia->asist_anio =date("Y", strtotime($request->fecha));
    ...
    public function asistenciaStore(Request $request){          
        ...
        if($fecha_ultAsistencia != date('Y-m-d')){
            ...
            $marcaje = new Marcaje();
            $marcaje->practicante_id = $practicante->id;
            $marcaje->marcaje = 1;
            $marcaje->turno = 0;
            $marcaje->save();
        }
        ...
        if($marcaje_datos->marcaje <= $nro_turnos*2){
            if($estado_ultAsistencia==0 && count($ultimaAsistencia)!=0 && $fecha_ultAsistencia == date('Y-m-d')){
                    ...
                    if($asistenciaHr_s > $horarioD_s){   
                        $asistencia = Asistencia::where('id',$ultimaAsistencia[0]->id)->first();       
                        $asistencia->h_salida = date("H:i:s");   
                        $asistencia->estado = 1;              
                        $asistencia->save();

                        $marcaje = Marcaje::where('practicante_id',$practicante->id)->first();       
                        $marcaje->marcaje = $marcaje->marcaje+1;   
                        $marcaje->turno = $marcaje->turno+1;      
                        $marcaje->save();
                    }else{
                ...
                $asistencia = new Asistencia();
                $asistencia->practicante_id=$practicante->id;
                $asistencia->horario_details_id = $horario_details[$marcaje_datos->turno]->id;    
                $asistencia->h_entrada = date("H:i:s");
                $asistencia->h_salida = "";
                ...
                if($asistenciaHr_e > $horarioD_e){    
                    ...
                    $asistencia->retraso = $difference->format('%H:%I:%S');
                }else{
                    $asistencia->retraso = "";
                }   
                    
                $asistencia->asist_fecha = date('Y-m-d');
                $asistencia->estado = 0;
        
                $asistencia->asist_mes =date("F", strtotime(date('Y-m-d')));
                $asistencia->asist_anio =date("Y", strtotime(date('Y-m-d')));
    ...
    public function asistenciaDelete($id){
        ...
        $falta = new Falta();
        $falta->practicante_id= $asistencia->practicante_id;
        $falta->horario_details_id= $asistencia->horario_details_id;
        $falta->hr_entrada= '';
        $falta->falta_fecha= $asistencia->asist_fecha;
        $falta->falta_mes= date("F", strtotime(date($asistencia->asist_fecha)));
        $falta->falta_anio= date("Y", strtotime(date($asistencia->asist_fecha)));
